...
slot_cb(struct dpl_event *ev){
    ...
    if(dw1000_rng_request_delay_start(rng, node_address, dx_time, mode).start_tx_error){
    }else{
#ifdef TICTOC
        os_error_t err = os_sem_pend(&rng->sem, OS_TIMEOUT_NEVER); // Wait for completion of transactions 
        assert(err == OS_OK);
