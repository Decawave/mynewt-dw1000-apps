...
char dw_twr_nrng_data_buf[DW_DATA_BUF_LEN];
static char nrng_data_buf[DW_DATA_BUF_LEN];
...
void
app_nrng_encode(dw1000_nrng_instance_t * nrng, uint8_t seq_num, uint16_t base){
    ...
    assert(rc == 0);
    json_fflush();
    memcpy(dw_twr_nrng_data_buf, nrng_data_buf, DW_DATA_BUF_LEN);
    memset(nrng_data_buf, 0, DW_DATA_BUF_LEN);
}
