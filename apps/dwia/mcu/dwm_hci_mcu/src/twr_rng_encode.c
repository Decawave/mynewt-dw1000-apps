...
char dw_twr_rng_data_buf[DW_DATA_BUF_LEN];
static char rng_data_buf[DW_DATA_BUF_LEN];

#define JSON_BUF_SIZE (1024)
static char _buf[JSON_BUF_SIZE];
static uint16_t idx=0;

static void _twr_encode(twr_frame_t * frame);
static void _raz_encode(twr_frame_t * frame);
static void _diag_encode(struct _dw1000_dev_instance_t * inst);

static void
json_fflush(){
    _buf[idx] = '\0';
    printf("%s\n", _buf);
    strcat(rng_data_buf, _buf);
    idx=0;
}
...
/*!
 * @fn app_rng_encodestruct os_event * ev)
 *
 ...
 */
void
app_rng_encode(dw1000_rng_instance_t * rng) {
    ...
    switch(frame->code){
        ...
        default: printf(",error: \"Unknown Frame Code\"");
                 strcat(rng_data_buf, ",error: \"Unknown Frame Code\"");
    }
    ...
    assert(rc == 0);
    json_fflush();
    memcpy(dw_twr_rng_data_buf, rng_data_buf, DW_DATA_BUF_LEN);
    memset(rng_data_buf, 0, DW_DATA_BUF_LEN);
}

/*!
 * @fn _twr_encode(twr_frame_t * frame) {
 *
...
/*!
 * @fn _raz_encode(twr_frame_t * frame) {
 *
 * @brief JSON encoding twr_frames support the folowing json objects:
...
/*!
 * @fn _diag_encode(struct _dw1000_dev_instance_t * inst) {
 *
 * @brief JSON encoding twr_frames support the folowing json objects:
