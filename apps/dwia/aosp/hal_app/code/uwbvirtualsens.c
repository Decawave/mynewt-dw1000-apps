...
#define fprintf(x, ...) ALOGD(__VA_ARGS__)

int usage(void)
{
	fprintf(stderr, "uwbvirtualsens start [sensors.gmin.so]\n");
	fprintf(stderr, "uwbvirtualsens [activate | deactivate] sensor_id\n");
	fprintf(stderr, "uwbvirtualsens [start | stop | get] ranging\n");
...
static int number_of_events = 0;static int non_param_poll = 1;
...
*/
    ...
    int fd, sz;
    char *c = (char *) malloc(READ_BYTES_REQUEST_SIZE * 2 * sizeof(char));

    fprintf(stderr, "sending INIT command to slave from master\n");
    fd = open(DW1000_DEV_FILENAME, O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (fd < 0)
    {
        fprintf(stderr, "file open failed for %s Write mode\n", DW1000_DEV_FILENAME);
        perror("r1");
        exit(1);
    }

    fprintf(stderr, "calling write %s\n", SPI_INIT_COMMAND);
    sz = write(fd, SPI_INIT_COMMAND, strlen(SPI_INIT_COMMAND));

    fprintf(stderr, "fd = %d, strlen(SPI_INIT_COMMAND) = %d, It returned %d\n", fd, strlen(SPI_INIT_COMMAND), sz);

    close(fd);
    fprintf(stderr, "close done for write, before sleep for ");

    usleep(INIT_COMMAND_TO_READ_DELAY);

    fprintf(stderr, "After usleep, Reading bytes from the device file");
    fd = open(DW1000_DEV_FILENAME, O_RDONLY);
    if (fd < 0)
    {
        fprintf(stderr, "file open failed for %s Read mode\n", DW1000_DEV_FILENAME);
        perror("r1");
        exit(1);
    }

    fprintf(stderr, "File open for read done, fd=%d\n", fd);
    fprintf(stderr, "Those bytes are as follows: \n");
    sz = 1;
    while(sz != 0) {
        sz = read(fd, c, READ_BYTES_REQUEST_SIZE);
        c[sz] = '\0';
        fprintf(stderr, "sz=%d, %s", sz, c);
        usleep(READ_TO_READ_DELAY);
    }

    close(fd);

    fprintf(stderr, "returning from run_sensors_thread");
    return NULL;
}
...
#ifdef ANDROID
#define NAME_PREFIX "/vendor/lib/hw/"
#else
#define NAME_PREFIX "/tmp/"
#endif

#define SENS_SERVER_NAME NAME_PREFIX "uwb-virtual-sensor-hal.so"
...
static const char *hal_paths[] = {
	"/system/vendor/lib64/hw/uwb-virtual-sensor-hal.so",
	"/system/lib/hw/uwb-virtual-sensor-hal.so",
	"uwb-virtual-sensor-hal.so",
	"/lib/uwb-virtual-sensor-hal.so",
};
...
static int start_hal(int argc, char **argv)
{
	...
	fprintf(stderr, "HAL loaded: name %s vendor %s version %d.%d id %s\n",
